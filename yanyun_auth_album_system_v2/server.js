...
function getMyMember(userId) {
  return db.prepare(`
    SELECT id, user_id AS userId, nickname, phrase, avatar_url AS avatarUrl, role_title AS roleTitle
    FROM members
    WHERE user_id = ?
  `).get(userId);
}
...
/*
 * 上传图片：必须登录。
 * 普通成员永远只能上传到自己的 member_id。
 * 即使前端伪造 memberId，后端也会强制改成自己的。
 * 管理员可以指定 memberId，方便代传。
 */
